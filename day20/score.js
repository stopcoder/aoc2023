...
class BroadCaster extends Module {
	...
	onSignal(bHigh, from) {
		this.outputs.forEach(name => {
			...
			Module.pipeline.push([m, bHigh, this.name]);
		});
	}
}
	...
	onSignal(bHigh, from) {
		if (!bHigh) {
			...
			this.outputs.forEach(name => {
				...
				Module.pipeline.push([m, this.#on, this.name]);
			});
		}
	}
}
...
class Conjunction extends Module {
	...
	onSignal(bHigh, from) {
		...
		this.outputs.forEach(name => {
			...
			Module.pipeline.push([m, pulse, this.name]);
		})
	}
...
async function processLineByLine() {
	...
	for (let i = 0; i < repeat; i++) {
		...
		while (Module.pipeline.length) {
			...
			console.log(`${from} sends ${highPulse? "high" : "low"} pulse to ${instance ? instance.name : "not exist"}`);
